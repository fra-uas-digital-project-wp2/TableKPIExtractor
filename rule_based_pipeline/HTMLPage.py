...
class HTMLPage:
    ...
    def detect_split_items(self):
        ...
        for ij in all_words:
            ...
            print_verbose(3, '---> Split item ' + str(self.items[ij[0]]) + ' at word ' + \
                          str(ij[1]) + '(x1=' + str(self.items[ij[0]].words[ij[1] - 1].rect.x1) + '<-> x0=' + str(
                self.items[ij[0]].words[ij[1]].rect.x0) + \
    ...
    def identify_connected_txt_lines(self):
        ...
        for cur_x, cnt in self.left_distrib.items():
            ...
            for i in range(len(cur_lines) - 1):
                ...
                if ((next_y > cur_y + min(cur_item.height + max_allowed_spacing,
                                          2 * cur_item.height)) or  # too far apart
    ...
    def mark_other_text_components(self):
        ...
        for cur_x, cnt in self.left_distrib.items():
            ...
            for i in range(len(cur_lines) - 1):
                ...
                print_verbose(9, "----> cur_y, next_y , y_threshold = " + str(cur_y) + "," + str(next_y) + "," + str(
                    y_threshold))

                if (abs(cur_y - next_y) < y_threshold and self.items[cur_item_id].height > self.items[
                    next_item_id].height):
        ...
        for it in self.items:
            ...
            for jt in self.items:
                if (
                        it == jt or jt.category == CAT_RUNNING_TEXT or jt.category == CAT_HEADLINE or jt.category == CAT_OTHER_TEXT or jt.category == CAT_FOOTER):
                    continue  # we do not consider these ones (=> text is assumed to be isolated, even if any of these ones are near)
                cur_dist = Rect.raw_rect_distance(it.pos_x, it.pos_y, it.pos_x + it.width, it.pos_y + it.height,
                                                  jt.pos_x, jt.pos_y, jt.pos_x + jt.width, jt.pos_y + jt.height)
    ...
    def discover_table_column(self, initial_item):
        ...
        vitems = self.sort_out_non_vconnected_items(vitems, -1 if top is None else top.pos_y,
    ...
    def render_to_png(self, in_dir, out_dir):
        ...
        if (not RENDERING_USE_CLUSTER_COLORS):
            ...
            # table borders
            for t in self.tables:
                context.rectangle([(t.table_rect.x0, t.table_rect.y0), (t.table_rect.x1, t.table_rect.y1)],
                                  fill=table_bg_color, outline=(0, 0, 0, 255))
                first = True
                for r in t.rows:
                    if (first):
                        first = False
        ...
        for it in self.items:
            if it.next_id != -1 and it.category == CAT_RUNNING_TEXT:
                line_shape = [(it.pos_x + 5, int(it.pos_y + it.height / 2)), (self.items[it.next_id].pos_x + 5,
                                                                              self.items[it.next_id].pos_y + int(
                                                                                  self.items[it.next_id].height / 2))]
    ...
    def __repr__(self):
        res = "====>>> HTMLPage : No. = " + str(self.page_num) + ", Width = " + str(
            self.page_width) + ", Height = " + str(self.page_height)
        for i in range(len(self.items)):
    ...
    def parse_html_file(fonts_dir, htmlfile):
        ...
        pattern_div = re.compile(
            '<div class="txt" style="position:absolute; (left|right):([0-9]+)px; top:([0-9]+)px;">(<span id="f[0-9]+" style="font-size:[0-9]+px;vertical-align:.*;color:rgba\([0-9]+,[0-9]+,[0-9]+,[0-9]+\);">.*</span>)</div>')
        pattern_span = re.compile(
            '<span id="f([0-9]+)" style="font-size:([0-9]+)px;vertical-align:.*;color:rgba\(([0-9]+),([0-9]+),([0-9]+),([0-9]+)\);">(.*)')
        ...
        pattern_font_url = re.compile(
            r'@font-face { font-family: ff([0-9]+); src: url\("([0-9]+\.ttf|([0-9]+)\.otf)"\); }')
                ...
                for s in spans:
                    if pattern_span.match(s):
                        ...
                        if int(gs[0]) in font_dict and font_dict[int(gs[0])] in font_url_dict:
                            span_font = ImageFont.truetype(fonts_dir + '/' + font_url_dict[font_dict[int(gs[0])]],
                                                           int(gs[1]))
                        ...
                        for b in bboxes:
                            if pattern_bbox.match(b):
                                ...
                                if (
                                        word.rect.x0 < word.rect.x1 and word.rect.y0 < word.rect.y1):  # otherwise, bad word!
