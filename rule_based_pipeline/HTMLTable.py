...
class HTMLTable:
    ...
    # this is not related with an actual table in HTML syntax
    ...
    headline_idx = None
    special_idx = None  # e.g., annotations
    table_rect = None
    ...
    def is_row_mergable(self, r0):  # return True iff row r0 and r0+1 can be merged
        ...
        y1_min = 9999999
        for j in range(self.num_cols):
            if (self.has_item_at(r0, j) and self.has_item_at(r0 + 1, j) and not self.get_item(r0, j).is_mergable(
                    self.get_item(r0 + 1, j))):
                return False
        ...
        while (c0 < self.num_cols and c1 < self.num_cols):
            if (self.has_item_at(r0, c0) and self.has_item_at(r1, c1)):
                ...
                if (min(x1_0, x1_1) - max(x0_0, x0_1) >= 0):
                    print_verbose(7, "-----> Can't merge rows " + str(r0) + " and next one, because c0=" + str(
                        c0) + ", c1=" + str(c1) + " would overlap")
                    print_verbose(7, "These are the items " + str(self.get_item(r0, c0)) + ", and " + str(
                        self.get_item(r1, c1)))
    ...
    def throw_away_cols_at_next_paragraph(self, paragraphs):
        ...
        for p in paragraphs:
            cnt = 0
            for it in self.items:
                if (it.pos_x == p and it.category in (CAT_RUNNING_TEXT,
                                                      CAT_HEADLINE) and it.pos_x < self.table_rect.x1 and it.pos_x + it.width > self.table_rect.x0):
                    if ((
                            it.pos_y < self.table_rect.y1 and it.pos_y + it.height * 5 > self.table_rect.y0) or  # TODO 5 was 10, test it
                            (
        ...
        for j in range(self.num_cols):
            ...
            print_verbose(7, "--> Col =" + str(j) + ", x0/x1=" + str(self.cols[j].x0) + "/" + str(
                self.cols[j].x1) + " belong to paragraph p_idx = " + str(cur_para_idx) + ", which is at " + str(
                my_paragraphs[cur_para_idx] if cur_para_idx != -1 else None) + " px")
    ...
    def identify_overlapping_special_items(self):
        ...
        def calc_single_col_boundary(tmp_idx, col0):
            ...
            for i in range(self.num_rows):
                ...
                if (cur_idx != -1):
                    cur_bdry = (min(cur_bdry[0], self.items[cur_idx].pos_x),
                                max(cur_bdry[1], self.items[cur_idx].pos_x + self.items[cur_idx].width))
            return cur_bdry
        ...
        def find_possible_overlapping_ix(c0, tmp_idx, boundaries):
            ...
            for j in range(self.num_cols):
                if boundaries[j][1] > bdry[0] and boundaries[j][0] < bdry[1]:
                    # all items from this col might be overlapping
                    for i in range(self.num_rows):
    ...
    def merge(tab1, tab2, page_width):  # note: tables must belong to same HTMLPage !
        ...
        def cols_are_mergable(tab1, tab2, col1, col2):
            ...
            for i in range(tab1.num_rows):
                ...
                c1_x1 = max(c1_x1, tab1.get_item(i, col1).pos_x + tab1.get_item(i, col1).width if tab1.has_item_at(i,
                                                                                                                   col1) else -1)

            for i in range(tab2.num_rows):
                c2_x0 = min(c2_x0, tab2.get_item(i, col2).pos_x if tab2.has_item_at(i, col2) else 9999999)
                c2_x1 = max(c2_x1, tab2.get_item(i, col2).pos_x + tab2.get_item(i, col2).width if tab2.has_item_at(i,
                                                                                                                   col2) else -1)
        ...
        def find_all_new_columns(tab1, tab2, tmp_rows, threshold_px):
            ...
            while (tab1_col < tab1.num_cols or tab2_col < tab2.num_cols):
                ...
                if (use_tab1 and use_tab2 and not cols_are_mergable(tab1, tab2, tab1_col,
                                                                    tab2_col)):  # TODO: and cols do not intersect by any rect
                    # cols cant be merged => use only first one
                    use_tab1 = tab1_col_x < tab2_col_x
                    use_tab2 = not use_tab1

                print_verbose(5, "------> tab1_col_x=" + str(tab1_col_x) + ", tab2_col_x=" + str(
                    tab2_col_x) + ", use1/2=" + str(use_tab1) + "/" + str(use_tab2))
                ...
                for i in range(num_rows):
                    ...
                    if (len(list_idx) > 1):
                        list_idx = sorted(list_idx,
                                          key=lambda id_i: tab1.items[tab1.idx[id_i[1]]].pos_y if id_i[0] == 1 else
                                          tab2.items[tab2.idx[id_i[1]]].pos_y)
                        for k in range(len(list_idx) - 1):
                            ...
                            if (not it.is_mergable(it1)):
                                ...
                                print_verbose(5, "-----> Split neccessary: " + str(it) + " cant be merged with " + str(
                                    it1))
                                print_verbose(5, "-----> Split is here: " + str(tmp_rows[0:i + 1]) + " <-> " + str(
                                    it1.pos_y) + " <-> " + str(tmp_rows[i + 1:]))
        ...
        while (tab1.count_marks(0) > 0 or tab2.count_marks(0) > 0):
            ...
            print_verbose(9, "----> Continue with idx1=" + str(idx1) + ",idx2=" + str(idx2) + ", r=" + str(
                cur_rect) + ", list_idx=" + str(list_idx))
    ...
    def is_non_overlapping_row_mergable(self, r0):  # return True iff row r0 and r0+1 can be merged
        ...
        if (n0 and n1):
            print_verbose(8,
                          '--->> is_non_overlapping_row_mergable: Rows r0=' + str(r0) + ' and r0+1 have both numbers')
        ...
        for j in range(self.num_cols):
            ...
            if (both_filled):
                ...
                if (self.get_item(r0, j).pos_y + self.get_item(r0, j).height * (3.0 if n0 else 2.1) < self.get_item(
                        r0 + 1, j).pos_y):
                    print_verbose(8, '--->> is_non_overlapping_row_mergable: Rows r0=' + str(
                        r0) + ' and r0+1 are too far apart. Items:' \
                                  + str(self.get_item(r0, j)) + ' and ' + str(self.get_item(r0 + 1, j)))
                    return False, 0
                cur_y0 = self.get_item(r0, j).pos_y + self.get_item(r0, j).height if self.has_item_at(r0, j) else 0
                cur_y1 = self.get_item(r0 + 1, j).pos_y if self.has_item_at(r0 + 1, j) else 9999999
        ...
        if (not has_mergable_candidates):
            print_verbose(8, '--->> is_non_overlapping_row_mergable: Rows r0=' + str(
                r0) + ' and r0+1 have no mergable candidates')
            return False, 0
        ...
        for j in range(self.num_cols):
            if ((self.has_item_at(r0, j) and self.get_item(r0, j).get_font_characteristics() != font_chars) \
                    or (self.has_item_at(r0 + 1, j) and self.get_item(r0 + 1,
                                                                      j).get_font_characteristics() != font_chars)):
                print_verbose(8, '--->> is_non_overlapping_row_mergable: Rows r0=' + str(
                    r0) + ' and r0+1 have different font chars')
                return False, 0

        mod_dist = math.floor((y1_min - y0_max) * 100.0) / 100.0 * (0.66 if n0 else 1)
        print_verbose(8, "---->>> For row=" + str(r0) + ", mod_dist=" + str(mod_dist) + ", y0_max=" + str(
            y0_max) + ",y1_min=" + str(y1_min))
