...
class KPIMeasure:
    ...
    def remove_duplicates(lst):
        # from the list of KPIMeasure "lst", remove all duplicates (same kpi_name and year), with less than best score
        # return lst # Don't remove anything (for debug purposes only)
        keep = [False] * len(lst)
        for i in range(len(lst)):
            ...
            for j in range(len(lst)):
                if i == j:
                    continue
                if (lst[j].kpi_name == lst[i].kpi_name and lst[j].year == lst[i].year and (
                        lst[j].score > lst[i].score or (lst[j].score == lst[i].score and j > i))):
