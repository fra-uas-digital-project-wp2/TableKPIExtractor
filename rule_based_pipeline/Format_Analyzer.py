...
class Format_Analyzer:
    pattern_numeric = re.compile(
        r'^\(?(-?\(?[ ]*[0-9]*(,[0-9][0-9][0-9])*(\.[0-9]+)?|-?[ ]*[0-9]*(\.[0-9][0-9][0-9])*(,[0-9]+)?)\)?(\*?)*$')  # by Lei
    pattern_numeric_multi = re.compile(
        r'^(\(?(-?\(?[ ]*[0-9]*(,[0-9][0-9][0-9])*(\.[0-9]+)?|-?[ ]*[0-9]*(\.[0-9][0-9][0-9])*(,[0-9]+)?)\)?)+$')
    pattern_year = re.compile(
        r'^[^0-9]*(19[8-9][0-9](/[0-9][0-9])?|20[0-9][0-9](/[0-9][0-9])?)[^0-9]*$')  # 1980-2099 #by Lei
    pattern_year_extended_1 = re.compile(
    ...
    def looks_percentage(val):
        return Format_Analyzer.looks_weak_numeric(val) and '%' in val
    ...
    def looks_weak_non_numeric(val):
        ...
        return num_letters > 0 and num_letters > num_numbers and (
                (num_letters + num_others > 1 and num_numbers < (num_letters + num_others) * 2 + 1) or (num_letters + num_others > num_numbers))
    ...
    def looks_pagenum(val):
        return Format_Analyzer.pattern_pagenum.match(val.replace(' ', '')) and len(val) > 0 and val.replace(' ',
                                                                                                            '') != '0'
