...
class AnalyzerTable:
    ...
    table_hierarchy = None  # for each ix, a reference to the parent ix (or -1, if root)
    ...
    def get_depth(self, i, j, dir):
        ident_threshold = (3.0 / 609.0) * self.htmlpage.page_width if dir == HIERARCHY_DIR_UP else (
                                                                                                           3.0 / 609.0) * self.htmlpage.page_height
    ...
    def get_txt_nodes_above(self, r0, c0, include_special_items, break_at_number):
        ...
        while r > 0:
            ...
            if Format_Analyzer.looks_weak_non_numeric(txt):
                ...
                if break_at_number:
                    break  # a number or something that is no text
    ...
    def find_applicable_year_line(self, r0):
        ...
        if first_match is not None:
            ...
            for i in range(len(self.year_rows) - 1, -1, -1):
                if first_match.row_num > self.year_rows[i].row_num >= first_match.row_num - 7:  # max 7 lines above
    ...
    def find_applicable_unit_item(self, kpispecs, r0):
        ...
        items_idx = self.htmlpage.find_items_within_rect(search_rect, [CAT_HEADLINE, CAT_OTHER_TEXT, CAT_TABLE_DATA,
                                                                       CAT_TABLE_HEADLINE, CAT_TABLE_SPECIAL, CAT_MISC,
                                                                       CAT_FOOTER])
        match_idx = -1
        for i in items_idx:
        ...
        for i in items_idx:
            ...
            if (cur_dist < best_dist):
                for w in it.words:
                    ...
                    if cur_year is not None and min_year <= cur_year <= max_year:
                        best_year = cur_year
                        best_dist = cur_dist
    ...
    def find_kpi_with_direct_years(self, kpispecs, bonus):
        ...
        for i in range(self.get_num_rows()):
            ...
            txt_nodes = txt_nodes + ([
                                         previous_txt_node_with_no_values] if previous_txt_node_with_no_values != '' and previous_txt_node_with_no_values not in txt_nodes else [])
            ...
            for y, it in value_items.items():
                ...
                print_verbose(4, '---> Match: ' + str(kpi_measure) + ': score=' + str(score) + ',h_score=' + str(
                    h_score) + ',anywhere_match_score=' + str(anywhere_match_score) + ',bonus=' + str(
                    bonus) + ', multiplier=' + str(multiplier))
    ...
    def find_kpi_with_indirect_years(self, kpispecs, bonus):
        ...
        # normal score from actual items:
        res = []
        ...
        for fixed_left_column in fixed_left_cols:
            ...
            for i in range(self.get_num_rows()):
                ...
                print_verbose(5, 'Looking at row i=' + str(i) + ', txt_nodes_row=' + str(
                    txt_nodes_row) + ',fonz_size=' + str(font_size_row_node))
                ...
                for j in range(fixed_left_column + 1, self.get_num_cols()):
                    ...
                    print_verbose(5, '\n-> Looking at cell: row,col=' + str(value_row) + ',' + str(j) + ':' + str(
                        value_txt) + ', font_size=' + str(font_size_cell))
                    if (font_size_row_node is not None and (
                            font_size_cell < font_size_row_node / 1.75 or font_size_cell > font_size_row_node * 1.75)):
                    ...
                    if (kpi_year == -1):
                        ...
                        print_verbose(8, '..............-> max_add=' + str(max_add) + ', y1(old)=' + str(
                            it.pos_y) + ', y1(new)=' + str(search_rect.y1))
                        ...
                        if (kpi_year == -1):
                            ...
                            kpi_year = self.search_year_agressive(search_rect, self.default_year - 10,
                                                                  self.default_year, base_pos_x, base_pos_y,
                                                                  aggressive_year_pattern=True)
                        print_verbose(7, '.........-> year found=' + str(
        ...
        print_verbose(5, "===> found AT.indirect KPIs on Page " + str(self.htmlpage.page_num) + ": " + str(
            res) + "\n================================")
